<div class="card mb-3" id="post-{{ $post->id }}">
                        ...
                        @if (auth()->check() && auth()->id() !== $post->user_id)
                            <span class="nav-item small ms-2">
                                ...
                                <a href="#" onclick="toggleFollow({{ $post->user_id }}, this)"
                                    ...
                                    <i class="bi {{ $isFollowing ? 'bi-check-circle-fill' : 'bi-plus-circle-fill' }}"></i>
    ...
    <div class="card-body">
                ...
                <div class="insta-track">
                    @foreach ($post->images as $image)
                        <div class="insta-slide">
                            <img src="{{ asset($image->image) }}" data-image-id="{{ $image->id }}"
                                alt="Post Image">
                        </div>
                    @endforeach
                </div>
        ...
        @elseif ($post->images->count() === 1)
            <img class="img-fluid rounded w-100" src="{{ asset($post->images->first()->image) }}">
        @else
            <div class="text-muted text-center py-4">No image available</div>
        @endif
